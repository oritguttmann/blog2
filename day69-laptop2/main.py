    ...
    if logged_in:
        ...
        if form.validate_on_submit():
            new_comment = Comment(
                ...
                author=current_user,  # also can be replaced by: author_id = current_user.id
                parent_post=requested_post,  # also can be replaced by: post_id = post_id
            )
        ...
        curr_id = 0
        if form.validate_on_submit():
            flash("You need to log-in in order to comment on a blog.")
            return redirect(url_for("login"))
    return render_template("post.html", post=requested_post, log_in=logged_in, admin=curr_id, form=form)
